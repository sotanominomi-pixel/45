document.addEventListener('DOMContentLoaded', () => {
  ...
  [tabClock, tabStopwatch, tabAlarm, tabSettings].forEach((tab, idx)=>{
    tab.addEventListener('click',()=>{ setMode(['clock','stopwatch','alarm','settings'][idx]); });
  });
  ...
  function restore(){
    if(localStorage.getItem('nclock_hours')){
      customHours = Number(localStorage.getItem('nclock_hours'));
      slider.value = customHours;
      labelHours.textContent = (lang==='en')?`${customHours} h`:`${customHours} 時間`;
    }
