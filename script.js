...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const L = {
    ja: {
      ...
      'label.hours':'1日の長さ','footer':'設定は自動で保存されます。',
    ...
    en: {
      ...
      'label.hours':'Day length','footer':'Settings are saved automatically.',
  ...
  function setMode(m){
    ...
    else if(m==='stopwatch') tabStopwatch.classList.add('active');
    else if(m==='alarm') tabAlarm.classList.add('active');
    else if(m==='settings') tabSettings.classList.add('active');
  ...
  function renderLaps(){
    lapList.innerHTML = '';
    if(laps.length===0) lapList.innerHTML=`<div style="color:var(--muted);padding:8px">${lang==='en'?'No laps':'ラップなし'}</div>`;
    else laps.forEach((t,i)=>{
  ...
  swStart.addEventListener('click', ()=>{
    ...
    swStart.classList.toggle('btn-start',!running);
    swStart.classList.toggle('btn-stop',running);
