document.addEventListener('DOMContentLoaded', () => {
  ...
  const L = {
    ja: { 'tab.clock':'時計','tab.stopwatch':'ストップウォッチ','tab.alarm':'アラーム','tab.settings':'設定',
          ...
          'label.hours':'1日の長さ','footer':'設定は自動で保存されます。','msg.pickTime':'時刻を選択してください',
          'msg.invalidTime':'不正な時刻です','msg.alarmSound':'アラームが鳴りました' },
    en: { 'tab.clock':'Clock','tab.stopwatch':'Stopwatch','tab.alarm':'Alarm','tab.settings':'Settings',
          ...
          'label.hours':'Day length','footer':'Settings are saved automatically.','msg.pickTime':'Please pick a time',
          'msg.invalidTime':'Invalid time','msg.alarmSound':'Alarm' }
  };
  ...
  function tick(now){
    ...
    if(mode==='clock'){
      ...
      display.textContent=showSeconds?`${String(h).padStart(2,'0')}:${String(m).padStart(2,'0')}:${String(s).padStart(2,'0')}`:`${String(h).padStart(2,'0')}:${String(m).padStart(2
          ...
          if(lastTriggered!==keyNow){
            ...
            try{
              if(Notification.permission==='granted'){
                new Notification('N Clock',{body:`${t('msg.alarmSound')}: ${String(a.hour).padStart(2,'0')}:${String(a.min).padStart(2,'0')}`});
              }
            }catch(e){}
            playAlarmSound();
